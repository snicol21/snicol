    ...
    button.addEventListener('click', () => {
      clipboard.writeText(codeBlock.textContent).then(
        () => {
          ...
          button.blur();
          span.classList.remove('animate-ping-once'); // remove the class first
          void span.offsetWidth; // trigger reflow
          span.classList.add('animate-ping-once'); // add the class again
          setTimeout(() => {
            span.classList.remove('animate-ping-once');
            span.innerHTML = svg;
          }, 250);
        },
        (error) => {
