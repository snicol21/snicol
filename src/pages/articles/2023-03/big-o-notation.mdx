...
After completing
[Mosh Hamedani's Data Structures and Algorithms course](https://codewithmosh.com/p/data-structures-algorithms-part1),
I found it to be an incredibly valuable resource. With a finance degree and a
career that has taken some unexpected turns, I discovered a talent for software
development and programming. However, I have sometimes felt at a disadvantage
when unable to converse fluently with other developers. This course has helped
me to think like a programmer.

## What is it?

Big O notation is a fundamental concept in computer science that describes how
scalable an algorithm is or its runtime complexity. This notation provides a way
for developers to evaluate and compare different algorithms, helping them choose
the most efficient one for a given task. For those who are new to programming,
learning Big O notation can be challenging, but it's a crucial skill that is
often tested in interviews at big companies. Through the use of Big O notation,
developers can "think like a programmer" and gain a deeper understanding of how
to create efficient and scalable code.
...
- As the input grows, so do the number of operations, linearly. Our function
  needs to perform the same number of operations for every input.
...
exponential time complexity of O(2^n), because each call to fibonacci results in
two more calls, leading to an exponentially increasing number of function calls
as n gets larger.
